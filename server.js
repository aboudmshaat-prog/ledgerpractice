...
function parseLloyds(text) {
  ...
  const dts = [...text.matchAll(/Balance on (\d{2}) ([A-Za-z]+) (\d{4})/g)];
  const periodStart = dts[0] ? dts[0][1] + " " + dts[0][2] + " " + dts[0][3] : null;
  const periodEnd = dts[1] ? dts[1][1] + " " + dts[1][2] + " " + dts[1][3] : null;
  let ym = null;
  if (dts.length) { const last = dts[dts.length - 1]; const mo = MN[last[2]]; if (mo) ym = last[3] + "-" + String(mo).padStart(2, "0"); }
